...
exports.generatePDFDeliveryNote = async (req, res) => {
  try {
    ...
    doc.fontSize(18).text("ALBARÁN", { align: "center" }).moveDown();
    doc.fontSize(12).text(`Usuario: ${note.usuario?.nombre || note.usuario?.email || 'Usuario desconocido'}`);
    doc.text(`Cliente: ${note.cliente?.nombre || 'Cliente desconocido'}`);
    doc.text(`Proyecto: ${note.proyecto?.nombre || 'Proyecto desconocido'}`);
    doc.text(`Fecha: ${new Date(note.createdAt).toLocaleDateString()}`).moveDown();
    doc.fontSize(14).text("Líneas:").moveDown(0.5);
